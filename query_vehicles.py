...
#  設定區（修改這裡即可）
# =============================================================================
INPUT_FILE  = "20260510-0610_G_AVC8 list .csv"        # 支援 .xlsx / .xls / .csv / .txt
...
class VehicleQueryRunner:
    ...
    def __init__(self, input_file: str, plate_col: str, delay_sec: float, threads: int = 3):
        ...
        src = Path(input_file)
        self.output_file     = str(src.parent / (src.stem + "_result.csv"))
        self.checkpoint_file = str(src.parent / (src.stem + "_checkpoint.json"))
    ...
    def run(self):
        ...
        reader = FileReader(self.input_file, self.plate_col)
